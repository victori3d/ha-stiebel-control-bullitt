   ...
   switch (Type)
   {
     ...
     case et_bool:
       ...
       if (!Value)
         ...
         strcpy(Val, "?");
       break;
    //Anpassung Patrick Best - Wärmepumpen Status
     case et_wpstatus:
       if ((Value & 0xff) == 0 && (Value >> 8) <= (int) High(WPStatusList))
         strcpy(Val, WPStatusList[Value >> 8].Name);
       else
         strcpy(Val, "?");
       break;
...
int TranslateString(const char * & str, unsigned char elster_type)
{
  ...
  switch (elster_type)
  {
    ...
    case et_wpstatus:
    {
      int s = High(WPStatusList);
      for ( ; s >= 0; s--)
        if (!strncmp(WPStatusList[s].Name, str, strlen(WPStatusList[s].Name)))
          break;
      
      if (s >= 0)
      {
        str += strlen(WPStatusList[s].Name);
        
        return WPStatusList[s].Index;
      }
      break;
    }
    case et_betriebsart:
